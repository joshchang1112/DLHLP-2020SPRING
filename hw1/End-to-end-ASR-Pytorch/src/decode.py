...
class BeamDecoder(nn.Module):
    ...
    def forward(self, audio_feature, feature_len):
        ...
        if self.ctc_w == 1.0:
            output_seq_tmp = ctc_output[0].argmax(dim = -1)
            output_seq = []
            flag = 0
            for i in range(len(output_seq_tmp)-1):
                if output_seq_tmp.data[i] != output_seq_tmp.data[i+1] and flag == 0:
                    output_seq.append(output_seq_tmp.data[i])
                    
                elif output_seq_tmp.data[i] != output_seq_tmp.data[i+1] and flag == 1:
                    flag = 0
                elif output_seq_tmp.data[i] == output_seq_tmp.data[i+1] and flag == 0:
                    output_seq.append(output_seq_tmp.data[i])
                    flag = 1
                else:
                    continue
            if output_seq_tmp.data[-1] != output_seq_tmp.data[-2]:
                output_seq.append(output_seq_tmp.data[-1])
            
            output_seq = torch.IntTensor(output_seq)
