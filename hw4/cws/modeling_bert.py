...
class BertForTokenClassification(BertPreTrainedModel):
    ...
    Examples::
        ...
        paraphrase_classification_logits = outputs[0]
        ...
        for i in range(len(paraphrase_results)):
            ...
            if paraphrase_results[i] == 2:
                sentence_length = sentence_length + len(paraphrase_results[i-length:i+1])
                length = 0

        # print(outputs[0].squeeze(0).shape)
        # print(len(input_ids[0]) - 2, sentence_length)
        # exit()
        # print(sentence_length)
        # print(len(input_ids[0]))
        # Viterbi
        # print(outputs[0].shape)
        if sentence_length != len(input_ids[0]) - 2:
            matrix = torch.tensor([[-float('Inf'), 0, 0, -float('Inf')], [-float('Inf'), 0, 0, -float('Inf')], [0, -float('Inf'), -float('Inf'), 0], [0, -float('Inf'), -float('Inf'), 0]]).cuda()
            for i in range(outputs[0].squeeze(0).shape[0]):
                if i == 0 or i == outputs[0].squeeze(0).shape[0] - 1:
                    continue
                if i == 1:
                    _, now = torch.topk(outputs[0][0][i] + matrix[3], 1)
                    outputs[0][0][i] = outputs[0][0][i] + matrix[3]
                else:
                    _, now = torch.topk(outputs[0][0][i] + matrix[previous], 1) 
                    outputs[0][0][i] = outputs[0][0][i] + matrix[previous]
